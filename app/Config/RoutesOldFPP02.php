...
$routes->group('admin', ['filter' => 'auth'], function($routes) {
     ...
     * GUESTS MANAGEMENT
     * ========================= */
    $routes->get('guests', 'Admin\Guests::index', ['filter' => 'role:admin,fo,hk']);

    // Datatable
    $routes->post('guests/datatable', 'Admin\Guests::datatable', ['filter' => 'role:admin,fo,hk']);
    
    $routes->get('guests/room-devices', 'Admin\Guests::roomDevices', ['filter' => 'role:admin,fo,hk']);
    $routes->post('guests/checkout', 'Admin\Guests::checkout', ['filter' => 'role:admin,fo,hk']);

    // CRUD AJAX
    $routes->post('guests/store', 'Admin\Guests::store', ['filter' => 'role:admin,fo,hk']);
    $routes->get('guests/edit/(:num)', 'Admin\Guests::edit/$1', ['filter' => 'role:admin,fo,hk']);
    $routes->post('guests/update', 'Admin\Guests::update', ['filter' => 'role:admin,fo,hk']);

    // Soft delete & restore
    $routes->post('guests/delete', 'Admin\Guests::deleteAjax', ['filter' => 'role:admin,fo,hk']);
    $routes->post('guests/restore', 'Admin\Guests::restore', ['filter' => 'role:admin,fo,hk']);
