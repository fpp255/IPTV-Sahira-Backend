...
$routes->group('admin', ['filter' => 'auth'], function($routes) {
     ...
     * GUESTS MANAGEMENT
     * ========================= */
    $routes->get('guests', 'Admin\Guests::index', ['filter' => 'role:admin,fo']);

    // Datatable
    $routes->post('guests/datatable', 'Admin\Guests::datatable', ['filter' => 'role:admin,fo']);
    
    $routes->get('guests/room-devices', 'Admin\Guests::roomDevices', ['filter' => 'role:admin,fo']);

    // CRUD AJAX
    $routes->post('guests/store', 'Admin\Guests::store', ['filter' => 'role:admin,fo']);
    $routes->get('guests/edit/(:num)', 'Admin\Guests::edit/$1', ['filter' => 'role:admin,fo']);
    $routes->post('guests/update', 'Admin\Guests::update', ['filter' => 'role:admin,fo']);

    // Soft delete & restore
    $routes->post('guests/delete', 'Admin\Guests::deleteAjax', ['filter' => 'role:admin,fo']);
    $routes->post('guests/restore', 'Admin\Guests::restore', ['filter' => 'role:admin,fo']);
     ...
     * TV MANAGEMENT
     * ========================= */
    $routes->get('tv-devices', 'Admin\TvDevices::index', ['filter' => 'role:admin,eng']);
    $routes->post('tv-devices/datatable', 'Admin\TvDevices::datatable', ['filter' => 'role:admin,eng']);
    $routes->post('tv-devices/save', 'Admin\TvDevices::save', ['filter' => 'role:admin,eng']);
    $routes->delete('tv-devices/delete/(:num)', 'Admin\TvDevices::delete/$1', ['filter' => 'role:admin,eng']);

    /* =========================
     * HK ITEMS
     * ========================= */
    $routes->get('hkitems', 'Admin\Hkitems::index', ['filter' => 'role:admin']);
    // Datatable
    $routes->post('hkitems/datatable', 'Admin\Hkitems::datatable', ['filter' => 'role:admin']);
    $routes->get('hkitems/create', 'Admin\Hkitems::create', ['filter' => 'role:admin']);
    $routes->post('hkitems/store', 'Admin\Hkitems::store', ['filter' => 'role:admin']);
    $routes->get('hkitems/show/(:num)', 'Admin\Hkitems::show/$1', ['filter' => 'role:admin']);
    $routes->post('hkitems/update/(:num)', 'Admin\Hkitems::update/$1', ['filter' => 'role:admin']);
    $routes->post('hkitems/delete/(:num)', 'Admin\Hkitems::delete/$1', ['filter' => 'role:admin']);

    /* =========================
     * HK CATEGORIES
     * ========================= */
    $routes->get('hk-categories', 'Admin\HkCategories::index', ['filter' => 'role:admin']);
    $routes->post('hk-categories/datatable', 'Admin\HkCategories::datatable', ['filter' => 'role:admin']);
    $routes->post('hk-categories/store', 'Admin\HkCategories::store', ['filter' => 'role:admin']);
    $routes->post('hk-categories/update/(:num)', 'Admin\HkCategories::update/$1', ['filter' => 'role:admin']);
    $routes->post('hk-categories/delete/(:num)', 'Admin\HkCategories::delete/$1', ['filter' => 'role:admin']);
    $routes->post('hk-categories/sort', 'Admin\HkCategories::sort', ['filter' => 'role:admin']);
    $routes->get('hk-categories/sort', 'Admin\HkCategories::sortView', ['filter' => 'role:admin']);

    /* =========================
     * HK ORDERS
     * ========================= */
    $routes->get('hkorders', 'Admin\Hkorders::index', ['filter' => 'role:admin']);
    $routes->post('hkorders/datatable', 'Admin\Hkorders::datatable', ['filter' => 'role:admin']);
    $routes->get('hkorders/(:num)', 'Admin\Hkorders::show/$1', ['filter' => 'role:admin']);
    $routes->post('hkorders/status/(:num)', 'Admin\Hkorders::updateStatus/$1', ['filter' => 'role:admin']);
    $routes->get('hkorders/detail/(:num)', 'Admin\Hkorders::detail/$1', ['filter' => 'role:admin']);
});
